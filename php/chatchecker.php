...
require 'dbcon.php'; 
$Get_Messages=
"select * from (select * from ".$_SESSION["CTN"]." where sender is not null
 order by num desc
...
if($GetM === false){trigger_error('Wrong SQL: ' . $Get_Messages . ' Error: ' . $con->error, E_USER_ERROR);
}else{$GetM->data_seek(0);
	$i=0;$j=0;while ($row = $GetM->fetch_assoc()){$messages[$i]=$row['sender_id'];
		$i+=1;
